...
func (s *Server) accountWorkbenchLoginInput(c *gin.Context) {
	if !s.workbenchReady(c) {
		return
	}
	var input accountworkbench.LoginInput
	if bindRequestJSON(c, &input) != nil {
		writeError(c, http.StatusUnprocessableEntity, "验证内容无效")
		return
	}
	if err := s.accountWorkbench.SubmitLoginInput(c.Request.Context(), workbenchOwner(c), c.Param("id"), c.Param("item"), input); err != nil {
		writeError(c, http.StatusConflict, err.Error())
		return
	}
