...
func (s *Service) importItem(ctx context.Context, value *privateRun, index int, payload map[string]any) error {
	...
	if text(current["status"]) != "inactive" || current["schedulable"] != false || !sameAccountIdentity(current, item.Credentials) {
		return errors.New("账号隔离状态或身份与导入内容不符")
	}
...
func (s *Service) promoteItem(ctx context.Context, value *privateRun, index int) error {
	...
	if accountVersion(current) != value.AccountVersions[id] || current["schedulable"] != false || text(current["status"]) != "inactive" || !sameAccountIdentity(current, value.Items[index].Credentials) {
