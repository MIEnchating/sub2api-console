...
动画检测会消耗实际 API 用量，只用于观察输出，不修改模型行为画像结论、健康评分、熔断或调度权重。仅支持已绑定稳定上游 Key 的 OpenAI / Anthropic 接口账号；人工优先账号可发起常规检测和动画检测，检测不会修改人工优先设置。浏览器只展示后端校验后的 SVG 图片，不执行模型生成的脚本；外部资源、HTML、事件属性和过大内容会被拒绝。
