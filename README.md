...
New API 模型价格页对 DeepSeek、Kimi（含 Moonshot 名称）、MiniMax、GLM、Qwen 家族采用官方来源。Go 后端分别读取 [DeepSeek 中文定价](https://api-docs.deepseek.com/zh-cn/quick_start/pricing)、[Kimi 模型定价目录](https://platform.kimi.com/docs/pricing/chat)、[MiniMax 按量计费](https://platform.minimaxi.com/docs/guides/pricing-paygo)、[GLM API 定价](https://docs.bigmodel.cn/cn/guide/start/pricing) 和 [Qwen 百炼定价](https://help.aliyun.com/zh/model-studio/model-pricing)。只采用官网明确列出的模型 ID、大小写变体和明确公布的兼容别名，不推测日期版本或渠道前缀；未列出或尚不支持计价单位的型号标为缺失，不再套用第三方旧价。其他厂商仍读取公开远程价卡，缺失时通过 Sub2API 管理接口获取默认价格。所有价格保留来源数值，不做币种换算；每 Token 与每百万 Token 仅按计价单位换算。

DeepSeek 分别保留峰谷输入、输出、缓存读取价格，以及北京时间工作日的半开时段 `[开始, 结束)`。Kimi 从官方目录发现各模型定价页；MiniMax 采用标准服务当前价（不含 `priority` 优先服务）；GLM 保留输入长度及输出长度阶梯；Qwen 采用华北 2（北京）实时 Token 价格及页面标明的折扣，保留上下文与思考/非思考档位，思考档以请求的 `enable_thinking=true` 匹配。Qwen 的缓存折扣需另行配置，不从正文提及的“享有折扣”推测单价；音频、图像等不同计费单位不会按文本 Token 误读。
