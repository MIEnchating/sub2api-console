...
上游鉴权恢复支持 Sub2API 邮箱/密码登录页及同站点验证，允许加载 `challenges.cloudflare.com` 的验证资源。账号工作台另提供固定 OpenAI 官方 OAuth 授权入口，不接受任意跨站 OAuth 地址。浏览器请求只允许公开 HTTPS 上游，浏览器与 API 需保持一致的出口网络；若单独给某个容器配置代理或不同出口，需要先统一出口。默认不持久化浏览器会话，后续 Token 续签仍由 Go 后端执行。

验证画面在前台等待时最多每秒读取四帧，输入不等待截图，连续排队的文字合并发送；关闭后丢弃在途画面。遇到验证码错误可用“刷新验证页面”重新 GET 登录页，不重放登录提交。已识别的 Cloudflare 数字错误码随画面返回并提示下一步，不回传页面控制台日志。`110600` / `110620` 应检查验证时限、服务器时钟与负载，`200500` 应检查验证站点连通性，`300xxx` / `600xxx` 表示挑战失败，不能据此认定密码错误。Cloudflare 不保证受控浏览器能通过生产验证；持续拒绝时需联系上游检查出口或使用上游支持的接口鉴权方式。
