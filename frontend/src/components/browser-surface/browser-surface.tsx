...
import type { BrowserFrame, BrowserInput } from "@/api";
...
export function BrowserSurface(props: {
  session: BrowserFrame;
  disabled: boolean;
  onInput: (value: BrowserInput) => Promise<unknown>;
}): ReactElement {
  const [text, setText] = useState("");
  const send = (value: BrowserInput): void => {
    void props.onInput(value).catch(() => undefined);
  };
  return (
    <div className="grid min-w-0 gap-3">
      <p className="text-muted-foreground text-sm" id="browser-surface-controls">
      ...
      <div className="overflow-x-auto rounded-md border">
        <div
          ...
          aria-label="上游登录页面"
          aria-describedby="browser-surface-controls"
