...
import { RefreshCw, Star, Trash2, Plus, Pencil, LayoutTemplate } from "lucide-react";
import { api } from "@/api";
...
export function TemplateList(): ReactElement {
  ...
  return (
    <section aria-label="配置模板" className="grid min-w-0 gap-4">
      ...
      {query.data && (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border bg-card p-4 text-sm">
      ...
      <div
        ...
        className={cn(
          ...
          (query.data?.items.length ?? 0) > 1 && "xl:grid-cols-2",
        )}
      >
        {query.data?.items.map((item) => (
          <article
            ...
            className={cn(
              "@container flex min-w-0 flex-col gap-4 rounded-xl border bg-card p-4",
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1 space-y-1.5">
                ...
                <p className="wrap-anywhere text-xs text-muted-foreground">
                  来源：
                  {item.source_id ? item.source_name || `账号 #${item.source_id}` : "手动创建"}
                </p>
                <p className="text-xs text-muted-foreground">
                  更新于 {new Date(item.synced_at).toLocaleString("zh-CN")}
                </p>
              </div>
            ...
            <TemplateDetails template={item} compact />
            <div className="flex flex-wrap gap-2 border-t pt-3">
              ...
              <Button
                variant="outline"
                disabled={pending}
                onClick={() => setManual({ template: item })}
              >
              ...
              {item.source_id && (
                <Button
                  variant="outline"
                  disabled={pending}
                  onClick={() => setEditor({ template: item })}
                >
                  <RefreshCw aria-hidden="true" />
                  重新同步
                </Button>
