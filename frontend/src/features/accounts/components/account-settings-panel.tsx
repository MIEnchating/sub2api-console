...
export function AccountSettingsPanel(props: {
  ...
  const allocation = useUpstreamAllocationDraft(
    "accounts",
    props.accountId,
    detail?.upstream_type?.toLowerCase() === "sub2api",
  );
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      ignoreCostWall: detail.ignore_cost_wall === true,
    });
  }, [detail, form, isDirty]);
  ...
  const save = useMutation({
    mutationFn: async (values: AccountSettingsValues) => {
      ...
      if (values.ignoreCostWall !== (detail.ignore_cost_wall === true)) {
        await api.setAccountIgnoreCostWall(props.accountId, values.ignoreCostWall);
      }
  ...
  const modelsButtonLabel = modelsLoading ? "获取中…" : "获取模型";
  return (
    <>
      <DialogBody className={accountDetailDialogLayout.body}>
        ...
        {detail ? (
          ...
          >
            ...
            <div className="grid min-w-0 content-start gap-5 border-t pt-5 md:border-t-0 md:border-l md:pt-0 md:pl-5">
              <section aria-labelledby={`${formId}-control`} className="grid gap-3">
                <SettingsSectionHeading
                  id={`${formId}-control`}
                  title="账号管控"
              ...
              <section
                aria-labelledby={`${formId}-independent`}
                className="grid gap-3 border-t pt-4"
              >
                <SettingsSectionHeading
                  id={`${formId}-independent`}
                  title="独立策略"
                  description="修改后点击保存生效。"
                />
                <div className="divide-border divide-y overflow-hidden rounded-lg border bg-muted/10">
                  <AccountCostWallSwitch
                    key={`cost-${props.accountId}`}
                    accountId={props.accountId}
                    enabled={form.watch("ignoreCostWall")}
                    onCheckedChange={(checked) =>
                      form.setValue("ignoreCostWall", checked, { shouldDirty: true })
                    }
                  ...
                  {detail.upstream_type?.toLowerCase() === "sub2api" ? (
                    <UpstreamAllocationSetting
                      key={`allocation-${props.accountId}`}
                      draft={allocation}
                      kind="accounts"
                      id={props.accountId}
                      disabled={save.isPending}
      ...
      <DialogFooter className="flex-row flex-wrap items-center justify-end">
        <p className="text-muted-foreground mr-auto w-full text-xs sm:w-auto">
          所有设置点击保存生效
        </p>
        ...
        <Button
          type="submit"
          form={formId}
          disabled={!detail || !allocation.ready || save.isPending}
        >
          <Save />
