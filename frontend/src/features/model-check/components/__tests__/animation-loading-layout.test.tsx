...
it("首次读取动画账号时按正式账号卡片网格和 360px 高度占位", () => {
  render(<AnimationAccountsSkeleton />);
  const status = screen.getByRole("status", { name: "正在读取账号" });
  expect(status).toHaveAttribute("aria-busy", "true");
  expect(status).toHaveClass("md:grid-cols-2", "lg:grid-cols-4", "min-w-0");
  expect(status.children).toHaveLength(4);
  for (const card of status.children) {
    expect(card).toHaveClass("h-[360px]", "min-w-0", "overflow-hidden");
